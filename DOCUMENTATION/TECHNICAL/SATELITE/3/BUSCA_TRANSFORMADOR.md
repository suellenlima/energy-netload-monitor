...
```python
from src.services.inpe_service import INPEService

inpe = INPEService(engine)

resultado = inpe.buscar_cbers4a_poligono(
    subestacao_id=1234,
    raio_km=1.5,
...
print(f"✅ Encontradas {len(resultado)} imagens")
```
...
for raio in raios:
    resultado = inpe.buscar_cbers4a_coordenadas(
        latitude=coordenada_lat,
        longitude=coordenada_lon,
        raio_km=raio
    )
    print(f"Raio {raio}km: {len(resultado)} imagens")
```
...
for trans_id in transformador_ids:
    resultado = inpe.buscar_cbers4a_poligono(
        subestacao_id=trans_id
    )
    resultados[trans_id] = resultado
    imagens = len(resultado) if isinstance(resultado, list) else resultado.get('imagens_encontradas', 0)
    print(f"T{trans_id}: {imagens} imagens")

# Estatísticas
total = sum(
    len(r) if isinstance(r, list) else r.get('imagens_encontradas', 0)
