...
### Backend - Serviços
- **`backend/src/services/inpe_service.py`** (UNIFICADO - v1)
  - 700+ linhas
  - Classes: `INPEService` (main), `BoundingBox`, `SatelliteMetadata`, `ImagemCBERS`
  - Métodos: 14 públicos + helpers privados
  - Consolidação: 3 arquivos antigos em 1 único arquivo
  - Aliases: `INPESatelliteService`, `INPEServiceV2`, `CBERSService`
...
### inpe_service.py
```
📍 INPEService (class principal)
   ├─ buscar_cbers4a_coordenadas(lat, lon, raio_km) → List[ImagemCBERS]
   ├─ buscar_cbers4a_poligono(subestacao_id, poligono_wkt) → Dict
   ├─ calcular_bbox_subestacao(lat, lon, raio_km) → BoundingBox
   ...
   ├─ armazenar_metadata_imagem(subestacao_id, metadata) → int | bool
   ├─ listar_imagens_subestacao(subestacao_id) → List[Dict]
   └─ criar_composicao_rgb(imagem_cbers, r, g, b) → bytes
...
📍 ImagemCBERS (dataclass)
   └─ Armazena dados de imagem CBERS-4A
