"""
SatelliteSourceService - Controle Inteligente de Fontes de Satélites
...
class SatelliteSourceService:
    """
    Serviço para gerenciar fontes de satélites com priorização e quota
    """
    ...
    def decidir_fonte_satelite_transformador(
        ...
        if preferencia == 'CBERS-4A':
            ...
            return {
                'fonte': 'CBERS-4A',
                'pode_usar': True,
                'motivo': 'CBERS-4A - busca por transformador, resolução 2m',
        ...
        if preferencia == 'GOOGLE_MAPS':
            ...
            if quota_info['pode_usar']:
                ...
                return {
                    'fonte': 'CBERS-4A',
                    'pode_usar': True,
                    'motivo': 'Fallback: Google Maps sem quota, usando CBERS-4A',
                    'transformador_id': transformador_id,
                    'resolucao_metros': 2.0,
                    'cobertura': 'Brasil/América do Sul'
                }
    ...
    def registrar_requisicao_cbers4a(
        ...
        try:
            ...
            with self.engine.begin() as conn:
                result = conn.execute(text("""
                    SELECT registrar_requisicao_cbers4a(
                        :subestacao_id,
                        :tipo_requisicao,
                        :status,
                        :data_imagem,
    ...
    def registrar_requisicao_google_maps(
        ...
        try:
            ...
            with self.engine.begin() as conn:
                result = conn.execute(text("""
                    SELECT registrar_requisicao_google_maps(
                        :subestacao_id,
                        :tipo_requisicao,
                        :status,
                        :bbox_min_lat,
