...
- Busca por TRANSFORMADOR (ponto + raio)
- Integração com SatelliteServiceV2 para tracking
...
class INPEServiceV2:
    ...
    def buscar_imagens_cbers4a_transformador(
        ...
        A área de busca (área poligonal) é obtida automaticamente do banco de dados.
        ...
        # 2. Obter área poligonal do banco de dados
        ...
        # 3. Calcular bbox ao redor do transformador usando a área poligonal
        bbox = self._calcular_bbox_raio(latitude, longitude, area_poligonal_km)
        if not bbox:
            return self._registrar_resultado_erro_transformador(
                transformador_id,
                'erro',
        ...
        try:
            ...
            search = self.catalog.search(
                collections=['CB4A-WPM-L4-DN-1'],  # CBERS-4A WPM Level-4 Digital Number (2m resolucao)
                bbox=bbox,
            ...
            for item in items:
                ...
                # Isso permite retornar imagens mesmo quando metadado nao esta disponivel
                if cloud_cover is None:
                    cloud_cover = 0  # Assumir sem nuvem se nao tiver dado
                else:
                    cloud_cover = float(cloud_cover)
                
                if cloud_cover <= cobertura_nuvem_max:
                    # Para CBERS-4A WPM, usar BAND0-4 (2m resoluçao)
                    # Assets: BAND0 (Blue), BAND1 (Green), BAND2 (Red), BAND3 (NIR), BAND4 (SWIR), thumbnail
                    band_red = None
                    band_green = None
                    band_blue = None
                    band_nir = None
                    ...
                    imagem_dict = {
                        ...
                        'sensor': 'CBERS-4A WPM',
                        'banda_pan': band_nir or best_url,  # NIR or best available
                        'banda_red': band_red or best_url,  # Red band
                        'banda_green': band_green or best_url,  # Green band
                        'banda_blue': band_blue or thumbnail_url,  # Blue band or thumbnail
                    }
            ...
            if self.satellite_service_v2:
                ...
                self.satellite_service_v2.registrar_requisicao_cbers4a(
                    ...
                    imagem_id=imagens[0].get('id') if len(imagens) > 0 else f"CBERS-4A_T{transformador_id}_0",
                    url_download=melhor_imagem_url,
                    observacoes=f"Transformador {transformador_id}: {len(imagens)} imagens encontradas"
                )
